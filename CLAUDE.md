...
# 릴리스 빌드 (공증 생략)
make release-unsigned VERSION=1.0.0
```
...
- **팝오버 푸터**: 갱신 주기 + 버전
- **설정**: Launch at Login (SMAppService), 업데이트 확인 (Sparkle), 버전/빌드 정보
...
- **자동 업데이트**: Sparkle 2 (EdDSA 서명, appcast.xml via GitHub Releases)
- **공증**: `xcrun notarytool` + Developer ID Application 인증서
- **릴리스 플로우**: `git tag v1.x.x && git push --tags` → GitHub Actions → 빌드+공증+DMG+appcast
- **Homebrew**: `brew tap l2juhan/tap && brew install --cask tokengauge` (별도 homebrew-tap 레포)
- **GitHub Secrets**: `DEVELOPER_ID_CERT_P12`, `DEVELOPER_ID_CERT_PASSWORD`, `KEYCHAIN_PASSWORD`, `APPLE_ID`, `APP_SPECIFIC_PASSWORD`, `SPARKLE_PRIVATE_KEY`
- **Info.plist**: `TokenGauge-Info.plist` (프로젝트 루트, Sparkle 설정 포함)
